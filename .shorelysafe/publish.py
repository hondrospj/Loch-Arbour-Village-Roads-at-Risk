...
def publish(app, source, apps, connection, dry_run=False):
    ...
    for i,sha in enumerate(plan['missing']):
        path, text_data = payloads[sha]
        data = text_data if text_data is not None else path.read_bytes()
        offset = 0
        while True:
            # Stay below common nginx request-size limits on WordPress hosts.
            ...
            offset=next_offset
        if (i+1)%100==0: print(f'{app["id"]}: uploaded {i+1}/{len(plan["missing"])} changed files',flush=True)
